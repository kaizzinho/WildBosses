...
__export(cobblemon_cache_exports, {
  MOD_ID: () => MOD_ID,
  allCachedMoves: () => allCachedMoves,
  allCachedSpecies: () => allCachedSpecies,
  moveByID: () => moveByID,
...
const cobbleMoves = /* @__PURE__ */ new Map();
const cobblemonSpecies = /* @__PURE__ */ new Map();
function registerMove(moveId, move) {
  cobbleMoves.set(moveId, eval(`(${move})`));
  return move;
}
...
function moveByID(id) {
  return cobbleMoves.get(id);
}
function speciesByID(id) {
  return cobblemonSpecies.get(id);
}
function allCachedMoves() {
  return Array.from(cobbleMoves.values());
}
function allCachedSpecies() {
