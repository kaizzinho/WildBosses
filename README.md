...
**[Modrinth](https://modrinth.com/mod/wildbosses) · [Releases](https://github.com/kaizzinho/WildBosses/releases) · [Source](https://github.com/kaizzinho/WildBosses) · [Issues](https://github.com/kaizzinho/WildBosses/issues)**
...
### Cobblemon 1.8 spawning compatibility

Natural Boss promotion is intentionally limited to Cobblemon's ordinary player/world spawner. Alpha Pokémon are excluded so native Alpha scaling and WildBoss scaling cannot stack accidentally. Fishing encounters, Poké Snack encounters, and activated Habitat Block spawns are also excluded for the initial 1.8 port.

Ordinary world spawns inside habitat structures remain eligible because they still come through the normal world-spawn path. Forced `/wb spawn` promotion remains available for deliberate testing and admin-created encounters.
...
#### Anti-cheese battle rules

Against an active WildBoss, a small set of bypass and extreme-stall moves fail while still consuming the player's action. This includes OHKO moves, Perish Song, Destiny Bond, Endeavor, Pain Split, fractional-HP attacks, Toxic, Leech Seed, Salt Cure, Haze, Clear Smog, Topsy-Turvy, Trick, and Switcheroo.

Curse is blocked only when used by a Ghost-type Pokémon. Normal non-Ghost Curse setup is still allowed. Ordinary non-Boss Cobblemon battles are not changed.
...
1. Download the latest WildBosses release from [GitHub Releases](https://github.com/kaizzinho/WildBosses/releases) or [Modrinth](https://modrinth.com/mod/wildbosses).
2. Install Fabric Loader for Minecraft `1.21.1`.
3. Install Fabric API, Fabric Language Kotlin, Cobblemon, and Cobblemon's required dependencies.
4. Place the WildBosses JAR in the `mods` folder.
5. Start the game or server once to generate the configuration.
6. Add optional integrations as desired.
...
- The current target is Minecraft `1.21.1` with Cobblemon `1.8.1+`.
...
- Dedicated-server behavior can vary with the exact modpack and optional integrations.
...
- continued validation against Cobblemon `1.8` spawn and battle lifecycle changes.
...
### Compatibilidade de spawn com Cobblemon 1.8

A promoção natural para Boss é limitada de propósito ao spawner normal de mundo ligado aos jogadores. Pokémon Alpha são excluídos para impedir que o escalonamento nativo de Alpha seja combinado acidentalmente com o escalonamento do WildBoss. Encontros de pesca, Poké Snack e spawns de Habitat Block ativado também ficam excluídos neste primeiro port para 1.8.

Spawns normais de mundo dentro das estruturas de habitat continuam elegíveis porque ainda usam o fluxo normal de spawn. A promoção forçada por `/wb spawn` continua disponível para testes deliberados e encontros criados por administradores.
...
#### Regras anti-cheese de batalha

Contra um WildBoss ativo, um conjunto pequeno de golpes de bypass e stall extremo falha enquanto ainda consome a ação do jogador. Isso inclui golpes de OHKO, Perish Song, Destiny Bond, Endeavor, Pain Split, ataques de HP fracionário, Toxic, Leech Seed, Salt Cure, Haze, Clear Smog, Topsy-Turvy, Trick e Switcheroo.

Curse é bloqueado apenas quando usado por um Pokémon do tipo Ghost. O setup normal de Curse em Pokémon não-Ghost continua permitido. Batalhas normais de Cobblemon sem WildBoss não são alteradas.
...
1. Baixe a versão mais recente do WildBosses pelo [GitHub Releases](https://github.com/kaizzinho/WildBosses/releases) ou [Modrinth](https://modrinth.com/mod/wildbosses).
2. Instale Fabric Loader para Minecraft `1.21.1`.
3. Instale Fabric API, Fabric Language Kotlin, Cobblemon e as dependências exigidas pelo Cobblemon.
4. Coloque o JAR do WildBosses na pasta `mods`.
5. Inicie o jogo ou servidor uma vez para gerar a configuração.
6. Adicione integrações opcionais se desejar.
...
- O alvo atual é Minecraft `1.21.1` com Cobblemon `1.8.1+`.
...
- O comportamento em servidor dedicado pode variar conforme o modpack e as integrações opcionais usadas.
